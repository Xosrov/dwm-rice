 ...
 */
 ...
 enum { SchemeNorm, SchemeSel, SchemeStatus, SchemeTagsSel, SchemeTagsNorm, SchemeTagsEmptySel, SchemeTagsEmptyNorm, SchemeInfoSel, SchemeInfoNorm }; /* color schemes */
 enum { NetSupported, NetWMName, NetWMState, NetWMCheck,
		NetSystemTray, NetSystemTrayOP, NetSystemTrayOrientation, NetSystemTrayOrientationHorz,
		NetWMFullscreen, NetActiveWindow, NetWMWindowType,
		NetWMWindowTypeDialog, NetClientList, NetWMWindowsOpacity, NetDesktopNames, NetDesktopViewport, NetNumberOfDesktops, NetCurrentDesktop, NetLast }; /* EWMH atoms */
 ...
 drawstatusbar(Monitor *m, int bh, char* stext) {
	 ...
	 while (text[++i]) {
		 if (text[i] == '^' && !isCode) {
			 ...
			 text[i] = '\0';
			 apply_fribidi(text);
			 w = TEXTW(fribidi_text) - lrpad;
			 drw_text(drw, x, 0, w, bh, 0, fribidi_text, 0);
			 // Indicator
			 if (text[i + 1] == 'd')
 ...
 drawbar(Monitor *m)
 {
	 ...
	 if (m == selmon) { /* status is only drawn on selected monitor */
		 apply_fribidi(stext);
		 tw = statusw = m->ww - drawstatusbar(m, bh, fribidi_text);
	 }
	 ...
	 for (i = 0; i < LENGTH(tags); i++) {
		 ...
		 // Modified code for empty workspace
		 if (occ & 1 << i)
			 drw_setscheme(drw, scheme[m->tagset[m->seltags] & 1 << i ? SchemeTagsSel : SchemeTagsNorm]);
		 else
			 drw_setscheme(drw, scheme[m->tagset[m->seltags] & 1 << i ? SchemeTagsEmptySel : SchemeTagsEmptyNorm]);
   ...
   if (display != NULL) {
	 ...
	 if (resm != NULL) {
	   ...
	   if (xrdb != NULL) {
		 ...
		 XRDB_LOAD_COLOR("dwm.tagsemptynormbgcolor", tagsemptynormbgcolor);
		 XRDB_LOAD_COLOR("dwm.tagsemptynormfgcolor", tagsemptynormfgcolor);
		 XRDB_LOAD_COLOR("dwm.tagsemptyselbgcolor", tagsemptyselbgcolor);
		 XRDB_LOAD_COLOR("dwm.tagsemptyselfgcolor", tagsemptyselfgcolor);
		 ...
		 XRDB_LOAD_COLOR("dwm.infoselfgcolor", infoselfgcolor);
